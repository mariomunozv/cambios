...
@$apoyoTecnico = $_POST['apoyoTecnico'];
@$titulo = $_POST['titulo'];
@$obtEspecializacion = $_POST['obtEspecializacion'];
@$uEspecializacion = $_POST['uEspecializacion'];
@$otroPerfeccionamiento = $_POST['otroPerfeccionamiento'];
@$cursoCapacitacion = $_POST['cursoCapacitacion'];
@$cursosImplementaProfesor = $_POST['cursosImplementa'];
@$tiempoCapacitando = $_POST['tiempoCapacitando'];
@$cursosCapacitandoProfesor = $_POST['cursosCapacitando'];
@$otroSingapur = $_POST['otroSingapur'];
@$otraInsSingapur = $_POST['otraInsSingapur'];
@$otroTipoCapacitacion = $_POST['otroTipoCapacitacion'];



if(isset($cursosCapacitandoProfesor)){
	$cursosCapacitando = implode(",",$cursosCapacitandoProfesor);
}

if(isset($cursosImplementaProfesor)){
	$cursosImplementa = implode(",",$cursosImplementaProfesor);
}
...
switch ($tipo){
	...
	$okDatos = actualizaDatosProfesorCl($rut,$nombre,$apellidoPaterno,$apellidoMaterno,
								$sexo,$fechaNacimiento,$telefono,$email,
								$anosExperiencia,$anosExperienciaColegioActual,$cursoActual,
								$apoyoTecnico,$titulo,$especializacion,$obtEspecializacion,
								$uEspecializacion,$otroPerfeccionamiento,$asignaturaACargoProfesor,
								$experienciaSingapur,$nivelES,$cursoCapacitacion,$cursosImplementa,
								$tiempoCapacitando,$cursosCapacitando,$otroSingapur,$otraInsSingapur,
		...
		)
	 {
	  if ($_FILES["file"]["error"] > 0)
	  ...
	  else
		{
			...
			dirigirse_a("fichaDocenteCl.php");
		}
