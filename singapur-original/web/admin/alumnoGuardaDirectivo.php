...
switch ($modo){
	...
	if (move_uploaded_file($_FILES['userfile']['tmp_name'], $carpeta.$nombre_archivo)){
		...
		for ($i=1;$i<count($alumnos);$i++){
			...
			guardarDirectivo($rutProfesor, $rbdColegio, $idTipoProfesor,$nombreProfesor, $apellidoPaternoProfesor, $apellidoMaternoProfesor, $sexoProfesor, $fechaNacimientoProfesor,$telefonoProfesor ,$emailProfesor,$anosExperienciaProfesor,$asignaturaACargoProfesor,$coordinadorEnlaceProfesor,$estadoAlumno);
			...
			$idUsuario = insertaUsuarioDirectivo($rutProfesor, $emailProfesor, $loginUsuario, $passwordUsuario, $estadoProfesor,"Directivo");
