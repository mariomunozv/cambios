...
function getAlternativas($idEnunciado){
	...
	$alternativas = array();
	while ($row = mysql_fetch_array($res)) {
		$alternativas[$i]=array("idAlternativa"=> $row["idAlternativa"],
...
// Revision
if (isset($_SESSION["sesionIdUsuarioRevisado"])){ // Otro usuario esta revisando la actividad
 		...
 		if($paginas[$j]["tipoActividadPagina"] == "Contenido"){
			...
			foreach($contenidos as $contenido){
				...
				switch($contenido["nombreTipoContenidoPagina"]){
					...
					case "Imagen":
					echo "<br>";
					echo "<img src=".$contenido["textoContenidoPagina"]."></img>";
					echo "<br>";
					break;
					...
					case "Formulario":
						...
						// Se debe desplegar el formulario ?? (INCOMPLETO)
				...
				//echo $contenido["nombreTipoContenidoPagina"]."<br>";
				//echo $contenido["textoContenidoPagina"]."<br>";
